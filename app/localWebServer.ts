...
export default class localServer {
	...
	private requestConnectCoGate ( socket: SocketIO.Socket, sessionHash: string, cmd: IConnectCommand ) {
	...
	private getTimelines ( socket: SocketIO.Socket, account: TwitterAccount, sessionHash: string, CallBack ) {
	...
	private postTweetViaQTGate ( socket, account: TwitterAccount, postData: twitter_postData, sessionHash: string, Callback ) {
	...
	private listenAfterTwitterLogin ( socket: SocketIO.Socket, sessionHash: string ) {
		...
		socket.on ( 'getTimelines', ( item: TwitterAccount, CallBack1 ) => {
			...
			return this.getTimelines ( socket, item, sessionHash, ( err, tweets: twitter_post ) => {
		...
		socket.on ( 'getTimelinesNext', ( item: TwitterAccount, maxID: number, CallBack1 ) => {
			CallBack1 ()
			return this.getTimelinesNext ( socket, item, maxID, sessionHash, ( err, tweets: twitter_post ) => {
				return this.tweetTimeCallBack ( socket, err, tweets, false )
